...
const typeToConverter = {
  ...
  UnaryExpression: (context, { operator, target }) =>
    `(${operator}${compileNode(context, target)})`,
  ...
  If: (context, { cond, block }) => `if (${compileNode(context, cond)}) ${
    compileNode(context, block)}`,

  ElseIf: (context, { cond, block }) => `else if (${compileNode(context, cond)}) ${
    compileNode(context, block)}`,

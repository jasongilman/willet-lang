...
const isThrowValueSeq = (node) =>
  astHelper.nodeType(node) === 'ValueSequence' &&
  node.getIn(['values', 0, 'type']) === 'Reference' &&
...
const quoter = (value) => {
  ...
  else if (Immutable.isKeyed(value)) {
    if (value.get('type') === 'Unquote') {
      node = value;
    }
    else {
      const properties = value.entrySeq().map(([k, v]) => dsl.property(k, quoter(v)));
...
const handleJSSpecialKeywordOperatorValueSeq = (context, node) => {
  ...
  if (values.getIn([0, 'type']) === 'Reference'
      && values.getIn([0, 'symbol'])
      && values.getIn([1, 'type']) === 'FunctionCall') {
...
const typeToConverter = {
  StaticJSCode: (context, node) => node.get('code'),

...
const typeToConverter = {
  ...
  InfixExpression: ({ left, operator, right }) =>
    `(${compileNode(left)} ${_.get(infixOperatorMap, operator, operator)} ${compileNode(right)})`,

  UnaryExpression: ({ operator, target }) => `(${operator}${compileNode(target)})`,

  RestAssignment: ({ item }) => `...${compileNode(item)}`,
  ...
  FunctionArgument: ({ arg, theDefault }) => {
    let code = compileNode(arg);
    if (theDefault) {
      code = `${code} = ${compileNode(theDefault)}`;
    }
    return code;
  },

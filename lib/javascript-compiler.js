...
// const quoter = (expression) => {
//   const visitor = (context, node) => {
//     if (astHelper.type === 'Unquote') {
//       node = astHelper.dsl.literal(node);
//     }
//     return node;
//     // if (astHelper.type !== 'Unquote') {
//     //   node = astHelper.dsl.literal(node);
...
//   };
//
//   return astHelper.postwalk(null, visitor, expression);
// };
...
const typeToConverter = {
  ...
  // TODO async
  Function: ({ _async, args, statements }) => `(${compileAndJoin(args)}) => ${
  ...
  Quote: ({ expression }) => compileNode(quoter(expression)),

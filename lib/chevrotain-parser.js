...
class WilletParserEmbedded extends EmbeddedActionsParser {
  constructor() {
    ...
    $.ArgumentDecl = $.RULE('ArgumentDecl', () => {
      ...
      const values = [];
      $.MANY(() => {
        values.push($.SUBRULE($.AssignmentTarget));
      });
      $.OPTION(() => {
        $.CONSUME(Spread);
        values.push(dsl.restAssignment($.SUBRULE($.symbol)));
      });
      $.CONSUME(RParen);
      return values;
    });
    ...
    $.ArrayDestructuring = $.RULE('ArrayDestructuring', () => {
      ...
      $.MANY(() => {
        items.push(orRules(
          'AssignmentTarget',
          // TODO I think this has to be the last one in javascript. Make it like the function
          // literal approach.
          'RestAssignment'
        ));
      });

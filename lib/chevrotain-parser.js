...
class WilletParserEmbedded extends EmbeddedActionsParser {
  constructor() {
    ...
    $.GetProperty = $.RULE('GetProperty', () => {
      ...
      const symbolToken = orRules('KeywordToken', 'symbol');
      return withChildPositions(
        dsl.getProperty(symbolToken.image),
        [startPos, symbolToken]
      );
    });
    ...
    $.GetPropertyImmutable = $.RULE('GetPropertyImmutable', () => {
      ...
      const symbolToken = orRules('KeywordToken', 'symbol');
      return withChildPositions(
        dsl.getPropertyImmutable(symbolToken.image),
        [startPos, symbolToken]
      );
    });
    ...
    $.stringLiteral = $.RULE('stringLiteral', () => {
      const consumed = $.CONSUME(StringLiteral);
      const stringLit = getImageAndPosition(consumed);
    ...
    $.stringLiteralSingle = $.RULE('stringLiteralSingle', () => {
      ...
      let converted = stringLit.image.slice(1, stringLit.image.length - 1)
    ...
    $.numberLiteral = $.RULE('numberLiteral', () => {
      const numStr = getImageAndPosition($.CONSUME(NumberLiteral));

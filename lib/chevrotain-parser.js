...
const Staticjs = lexer.tokenVocabulary.Staticjs;
...
class WilletParserEmbedded extends EmbeddedActionsParser {
  constructor() {
    ...
    $.NegatedExpression = $.RULE('NegatedExpression', () => {
      $.CONSUME(Not);
      const expr = $.SUBRULE($.NonInfixExpression);
      return dsl.negate(expr);
    });

    $.StaticJSCode = $.RULE('StaticJSCode', () => {
      $.CONSUME(Staticjs);
      $.CONSUME(LParen);
      const codeString = $.SUBRULE($.stringLiteral);
      $.CONSUME(RParen);
      return $.ACTION(() => dsl.staticJs(codeString.get('value')));
    });
    ...
    $.quote = $.RULE('quote', () => {
      ...
      $.OR([
        ...
        },
        {
          ALT: () => {
            $.CONSUME2(RParen);
            quoteResult = dsl.quoteWithBlock($.SUBRULE($.SoloBlock));
          }
        }
      ]);
      return quoteResult;
    ...
    $.MapDestructuring = $.RULE('MapDestructuring', () => {
        ...
        $.OR4([
          {
            ALT: () => {
              ...
              const key = orRules1('stringLiteral', 'numberLiteral');
              $.CONSUME1(Colon);
              const value = orRules2(
                'MapDestructuring',
                'ArrayDestructuring',
                'SymbolReference'
              );
      ...
      $.CONSUME(RCurly);
      return dsl.mapDestructuring(...props);
    });
    ...
    const arrayRule = (dslFunc) => () => {
      ...
      $.CONSUME(RSquare);
      return dslFunc(...values);
    };

    $.ArrayDestructuring = $.RULE('ArrayDestructuring', arrayRule(dsl.arrayDestructuring));
    $.ArrayLiteral = $.RULE('ArrayLiteral', arrayRule(dsl.array));

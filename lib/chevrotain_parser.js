  ...
  constructor() {
    ...
    $.FunctionLiteral = $.RULE('FunctionLiteral', () => {
      ...
      if (body) {
        ...
        if (!_.every(list, (item) => item.type === 'Reference')) {
          this.SAVE_ERROR(
            new chevrotain.MismatchedTokenException(
              'Only symbols can be used in function declaration'
            )
          );
        }
        const args = _.map(list, (item) => dsl.symbolAssignment(item.symbol));
    ...
    $.AssignmentTarget = $.RULE('AssignmentTarget', () => {
      ...
      // TODO ensure this workes because we can't have symbol above
      // if (_.isString(val)) {

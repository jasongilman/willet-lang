...
const nodeType = (node, defaultVal = null) =>
  ((node && node.get) ? node.get('type', defaultVal) : defaultVal);
...
const prewalk = (context, visitor, node) => prepostwalk(context, visitor, _.identity, node);

const postwalk = (context, visitor, node) => prepostwalk(context, _.identity, visitor, node);
...
const def = (defType, target, value) => l1.Def(
  value ? { defType, target, value } : { defType, target }
);

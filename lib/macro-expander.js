...
const runMacro = (context, statements, stmtIndex, macro) => {
  ...
  if (macro._wlt_macro_terms) {
    ...
    const [_ignored, { args, block }, ...rest] = node.values;
    // Call the macro with the ast
    const macroArgs = _.concat([block], args);
...
const createRequireFn = (context) => (requirePath) => {
  ...
  let fullPath;
  try {
    // TODO this doesn't work right now because the path is really just this directory.
...
expandMacros = (context, astRoot) => {
  ...
  // Insert willet core
  if (!context.skipCore) {
    result.statements.unshift(dsl.staticJs(CORE_END));
    ...
    result.statements.unshift(dsl.staticJs(CORE_START));
  }

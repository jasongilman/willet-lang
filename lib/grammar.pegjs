...
Parts based on JavaScript Grammar here https://github.com/pegjs/pegjs/blob/master/examples/javascript.pegjs
...
Program = __ stmts:TopLevelStatementList? __ {
  return {
    type: "Program",
    statements: stmts
  };
...
TopLevelStatementList "statements" =
	head:TopLevelStatement tail:(__ stmts:TopLevelStatement)*
	{
    return buildList(head, tail, 1);
	}
/* Macros can only be defined at the top level */
TopLevelStatement = Assignment / Def / Macro / Expression

StatementList "statements" =
	head:Statement tail:(__ stmts:Statement)*
	{
    return buildList(head, tail, 1);
	}

Statement = Assignment / Def / Expression

/* TODO is there a way to make mutation more explicit? */
...
Def
  = "def " __ s:AssignmentTarget __ "=" __ v:Expression
    { return { type: "Def", target: s, value:v }; }
  / "def " __ s:AssignmentTarget
    { return { type: "Def", target: s }; }

Macro = "defmacro" __ s:Symbol __ '=' __ f:FunctionLiteral {
  return {
    type: 'Macro',
    symbol: s,
    fn: f
...
Unquote = "unquote" __ "(" __ e:Expression __ ")" {
  return {
    type: 'Unquote',
    expression: e
  };
...
ArrayDestructuring
  = "[" __ targets:DestructuringList __ "]" {
     return { type: "ArrayDestructuring", targets: targets };
   }

DestructuringList
  = head:ArrayAssignmentTarget tail:(__ ArrayAssignmentTarget)* {
      return buildList(head, tail, 1);
    }

ArrayAssignmentTarget = AssignmentTarget / RestAssignment

RestAssignment = "..." s:Symbol { return { type: "RestAssignment", symbol:s } };
...
ValueReference = SymbolReference / Literal

SymbolReference = s:Symbol {
  return {
    type: "Reference",
    symbol: s
  };
...
ArrayLiteral
  = "[" __ "]" { return { type: "ArrayLiteral", values: [] }; }
  / "[" __ head:ArrayMember tail:(__ ArrayMember)* __ "]" {
    return { type: "ArrayLiteral", values: buildList(head, tail, 1) };
  }

ArrayMember = Spread / Expression

Spread = "..." s:(SymbolReference / ArrayLiteral) { return { type: "Spread", item: s}; }
...
PropertyAssignment
  ...
  /* TODO add support for properties with the name as the same as a reference */
  //  / ref:Reference { return   something like the above map with key and value set. }
...
ArgumentList "arguments"
  = e:ArgumentMember __ a:ArgumentList
	{
		a.unshift(e);
		return a;
	}
	/ e:ArgumentMember { return [e]; }

ArgumentMember = Spread / Expression

// Function Literal
FunctionLiteral "function"
  = async:"async"? "(" __ a:ArgumentDecl? __ ")" __ "=>" __ body:FunctionBody
	{
		return {
			type: "Function",
			async: async !== null,
...
WilletKeyword
  = "quote" !IdentifierPart
  / "unquote" !IdentifierPart
